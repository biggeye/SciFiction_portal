...
async function upload_avatar(image, name, description, userId, supabaseClient) {
  ...
  let { imageTableData, imageTableError } = await supabaseClient.rpc("add_avatar", {
    p_uuid: uuid,
    p_name: name,
    p_title: description,
    p_url: url,
...
async function upload_script(content, title, userId, supabaseClient) {
  // Generate UUID
  const uuid = uuidv4();
...
async function upload_voiceover(audio, name, title, userId, supabaseClient) {
  // Generate UUID
...
async function upload_video(result_url, name, duration, userId, supabaseClient) {
 ...
 const uuid = uuidv4();
  ...
  const { videoTableData, videoTableError } = await supabaseClient
    .rpc("add_video", {
    p_uuid: uuid,
    p_name: name,
    p_duration: duration,
